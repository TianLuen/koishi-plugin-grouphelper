    ...
    .action(async ({ session }, user, count = 1) => {
      ...
      if (warnCount >= ctx.config.warnLimit) {
        ...
        try {
          ...
          dataService.logCommand(session, 'warn', userId, `已警告 ${count} 次，累计 ${warnCount} 次，触发自动禁言 ${formatDuration(milliseconds)}`)
          ...
          dataService.logCommand(session, 'warn', userId, `已警告 ${count} 次，累计 ${warnCount} 次，但自动禁言失败：${e.message}`)
